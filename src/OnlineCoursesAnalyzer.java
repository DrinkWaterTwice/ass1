...
public class OnlineCoursesAnalyzer {
    ...
    public OnlineCoursesAnalyzer(final String datasetPath) {
    ...
    public Map<String, Integer> getPtcpCountByInstAndSubject() {
        ...
        LinkedHashMap<String, Integer> st = new LinkedHashMap<>(16,
            0.75f, true);
    ...
    public Map<String, List<List<String>>> getCourseListOfInstructor() {
        ...
        courses.stream().map(t -> new String[]{t.title,
            t.instructors}).forEach(t -> {
    ...
    //4
    public List<String> getCourses(final int topK, final String by) {
    ...
    //5
    public List<String> searchCourses(final String courseSubject, double percentAudited,
        final double totalCourseHours) {
        List<String> lis = new ArrayList<>(courses.stream()
            .filter(t -> t.subject.toLowerCase().contains(courseSubject.toLowerCase()))
            .filter(t -> t.percentAudited >= percentAudited)
    ...
    //6
    public List<String> recommendCourses(final int age, final int gender,
        final int isBachelorOrHigher) {
        ...
        Map<String, Double> ageMap = courses.stream().collect(
    ...
    public double getSimilar(final CourseByNumber course, final int age,
        final int gender, final int isBachelorOrHigher) {
        return Math.pow(age - course.getAvgAge(), 2) + Math.pow(
            gender * 100 - course.getTotalGender(), 2) + Math.pow(
            isBachelorOrHigher * 100 - course.getTotalIsBach() * 100, 2);
    }
...
class Course {
    ...
    public Course(final String institution, final String number, final Date launchDate,
        String title,
        String instructors, String subject, final int year,
        final int honorCode, final int participants, final int audited,
        final int certified,
        final double percentAudited, final double percentCertified,
        final double percentCertified50,
        final double percentVideo, final double percentForum,
        final double gradeHigherZero, double totalHours,
        final double medianHoursCertification, final double medianAge,
        final double percentMale, final double percentFemale,
        final double percentDegree) {
